...
public class UpdateCommand implements Command {
    ...
    @Override
    public void execute() {
        logger.info("User wants to update existing mix");
        ...
        for (Mix mix : mixes) {
            ConsoleHelper.writeMessage(String.format("Обновлеяется микс {}", mix.getNames()));
            mix = updateMix(mix);
            MixDAO.update(mix, session);

            logger.info("Successfully updated mix: {}", mix.getNames());
        }

        session.close();
    }

    private Mix updateMix(Mix mix) {
